  ...
  if (gltf.animations) {
    gltf.animations.forEach((anim, i) => {
      ...
      if (anim.channels) {
        ...
        // Look at the first few channels
        anim.channels.slice(0, 5).forEach((channel, cIdx) => {
          ...
          console.log(`    Channel ${cIdx}: targets node "${nodeName}" (index ${nodeIdx}) property "${target.path}"`);
        });
        if (anim.channels.length > 5) {
          console.log(`    ... and ${anim.channels.length - 5} more channels`);
        }
      }
    });
  }
